...
public class TransaccionFaker : Faker<TransaccionBaseDto>
{
    public TransaccionFaker(Cliente origen, Cliente destino, IEnumerable<Pais> paises)
    {
        ...
        RuleFor(t => t.IdEnvia, _ => origen.Id)  // Id aleatorio para el que envía
            ...
            .RuleFor(t => t.MonedaOrigen, paises.FirstOrDefault(p => p.Id == origen.PaisId).Divisa)  // Código de moneda de origen
            .RuleFor(t => t.MonedaDestino, paises.FirstOrDefault(p => p.Id == destino.PaisId).Divisa)  // Código de moneda de destino
